  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        deptData.forEach(d => { if (d.id) deptMap[d.id] = d.name; });
        setDepartmentsMap(deptMap);
        
        const desigMap: Record<string, string> = {};
        desigData.forEach(d => { if (d.id) desigMap[d.id] = d.title; });
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          <Avatar className="h-9 w-9">
                            {emp.photoUrl ? (
                              <img src={emp.photoUrl} alt="Profile" className="h-full w-full object-cover" />
                            ) : (
