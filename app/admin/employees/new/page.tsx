...
export default function AddEmployeePage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [deps, desigs, emps, rolesData, vendorsData] = await Promise.all([
  ...
  const onSubmit = async (data: CreateEmployeeInput) => {
    ...
    try {
      ...
      if (qrFile) {
        try {
          const storageRef = ref(storage, `companies/${companyId}/employees/${uid}/paymentQr.jpg`);
      ...
      if (aadharFile) {
        try {
          // preserving extension if possible, or just default to jpg
