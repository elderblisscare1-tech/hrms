...
export default function EditEmployeePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [deps, desigs, emps, rolesData, empDoc, vendorsData] = await Promise.all([
          ...
          listDocuments(companyId, "vendors")
        ]);
        setDepartments(deps);
        ...
        if (empDoc) {
          ...
          if (empDoc.paymentQrUrl) setCurrentQrUrl(empDoc.paymentQrUrl);
        }
      } catch (error) {
  ...
  const onSubmit = async (data: CreateEmployeeInput) => {
    ...
    try {
      ...
      let paymentQrUrl = currentQrUrl;
      if (qrFile) {
        try {
          const storageRef = ref(storage, `companies/${companyId}/employees/${id}/paymentQr.jpg`);
          await uploadBytes(storageRef, qrFile);
          paymentQrUrl = await getDownloadURL(storageRef);
        } catch (uploadErr) {
          console.error("Failed to upload QR image:", uploadErr);
        }
      }

      const cleanData = { ...data, photoUrl, paymentQrUrl };
      ...
      if (!cleanData.vendorId) delete cleanData.vendorId;
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-sm font-medium mb-1.5 block">Target Working Days/Month</label>
              <Input type="number" min="0" max="31" placeholder="e.g. 22" {...register("targetWorkingDays", { valueAsNumber: true })} />
            </div>
            <div>
            ...
            <div>
              <label className="text-sm font-medium mb-1.5 block">Vendor (If Contracted)</label>
              <select className="flex h-10 w-full rounded-lg border border-[hsl(var(--input))] bg-[hsl(var(--background))] px-3 py-2 text-sm" {...register("vendorId")}>
            ...
            <div className="md:col-span-2 pt-4 border-t border-[hsl(var(--border)/0.5)]">
              <h4 className="text-sm font-medium mb-2">Payment & Bank Details (Optional)</h4>
              <p className="text-sm text-[hsl(var(--muted-foreground))] mb-4">Provide any one of the following methods for salary/payments.</p>
              
              <div className="space-y-6">
                {/* Option 1: Bank Account */}
                <div className="p-4 border rounded-lg bg-[hsl(var(--muted)/0.3)]">
                  <h5 className="text-sm font-semibold mb-3">Option 1: Bank Account Details</h5>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <div>
                      <label className="text-sm font-medium mb-1.5 block">Bank Name</label>
                      <Input placeholder="HDFC, SBI..." {...register("bankDetails.bankName")} />
                    </div>
                    <div>
                      <label className="text-sm font-medium mb-1.5 block">Account Holder Name</label>
                      <Input placeholder="John Doe" {...register("bankDetails.accountName")} />
                    </div>
                    <div>
                      <label className="text-sm font-medium mb-1.5 block">Account Number</label>
                      <Input placeholder="1234567890" {...register("bankDetails.accountNumber")} />
                    </div>
                    <div>
                      <label className="text-sm font-medium mb-1.5 block">IFSC Code</label>
                      <Input placeholder="HDFC0001234" {...register("bankDetails.ifscCode")} />
                    </div>
                    <div>
                      <label className="text-sm font-medium mb-1.5 block">Branch Name</label>
                      <Input placeholder="Andheri East" {...register("bankDetails.branchName")} />
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center">
                  ...
                  <span className="px-3 text-xs text-[hsl(var(--muted-foreground))] uppercase font-medium">OR</span>
                  <div className="flex-1 border-t border-[hsl(var(--border))]"></div>
                </div>

                {/* Option 2: UPI ID */}
                <div className="p-4 border rounded-lg bg-[hsl(var(--muted)/0.3)]">
                  <h5 className="text-sm font-semibold mb-3">Option 2: UPI ID</h5>
                  <div className="max-w-md">
                    <label className="text-sm font-medium mb-1.5 block">UPI ID</label>
                    <Input placeholder="john@upi" {...register("bankDetails.upiId")} />
                  </div>
                </div>
                ...
                {/* Option 3: QR Code */}
                <div className="p-4 border rounded-lg bg-[hsl(var(--muted)/0.3)]">
                  <h5 className="text-sm font-semibold mb-3">Option 3: Payment QR Code</h5>
                  <div className="max-w-md">
                    <label className="text-sm font-medium mb-1.5 block">Upload QR Image</label>
                    <div className="flex items-center gap-4">
                      {currentQrUrl && !qrFile && (
                        <div className="h-10 w-10 border rounded overflow-hidden">
                          <img src={currentQrUrl} alt="QR" className="h-full w-full object-cover" />
                        </div>
                      )}
                      <div className="flex-1">
                        <Input 
