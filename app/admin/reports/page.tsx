...
export default function ReportsPage() {
  ...
  const generateMasterReport = async () => {
    ...
    try {
      const dates = eachDayOfInterval({ start: parseISO(startDate), end: parseISO(endDate) }).map(d => format(d, "yyyy-MM-dd"));
      ...
      if (options.dailyAttendance || options.mappingTab) {
        const attendancePromises = dates.map(date => 
          listDocuments<AttendanceRecord>(companyId, `attendance/${date}/records`)
        ...
        attendanceResults.forEach(({ date, records }) => {
          records.forEach(r => {
            ...
            attendanceMap.get(r.employeeId)![date] = r;
          });
        });
      }
      ...
      const employeesData = employees.map(emp => {
        ...
        if (options.dailyAttendance) {
          dates.forEach(date => {
            ...
            row[`Attendance (${date})`] = record ? record.status.toUpperCase() : "NOT MARKED";
          });
        }
      ...
      if (options.mappingTab) {
        const vendorEmployeeData = employees
          ...
          .map(emp => {
            ...
            dates.forEach(date => {
              const record = attendance?.[date];
              row[`Employee Attendance (${date})`] = record ? record.status.toUpperCase() : "NOT MARKED";
            });
      ...
      XLSX.writeFile(wb, `Customized_Report_${startDate}_to_${endDate}.xlsx`);
          ...
          <CardContent className="pt-6 space-y-6">
            <div className="space-y-4">
              ...
              <p className="text-xs text-[hsl(var(--muted-foreground))]">
                The attendance status (Present/Absent) will be fetched for all dates in this range.
              </p>
            </div>
