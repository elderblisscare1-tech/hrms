...
export default function AttendancePage() {
  ...
  const { data: attendanceRecords, loading: attLoading, mutate: mutateAttendance } = useCollection<AttendanceRecord>(companyId, `attendance/${selectedDate}/records`);
  ...
  [attendanceRecords, employeeFilter]);

  const { presentCount, absentCount, lateCount, leaveCount } = React.useMemo(() => {
  ...
  const handleDelete = async (recordId: string) => {
    if (confirm("Are you sure you want to delete this attendance record?")) {
      try {
        ...
        mutateAttendance(attendanceRecords.filter(r => r.id !== recordId), false);
      } catch (error) {
